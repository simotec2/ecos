...
async function evaluateCompetencyAI(question, answer, keywords) {
    ...
    if (length > 200)
        ...
        depthScore = 20;
    const finalScore = Math.round((keywordScore * 0.6) +
        (depthScore * 0.4));
    return {
...
async function generateAIReport(input) {
    ...
    const bottom = [...sorted]
        .reverse()
        .slice(0, 3);
    const strengths = top.map((c) => `${c.name} (${c.score}%)`).join(", ");
    const gaps = bottom.map((c) => `${c.name} (${c.score}%)`).join(", ");
    const riskText = input.score >= 85
...
El participante presenta un desempeño ${input.score >= 85
        ? "sólido y consistente"
        : input.score >= 55
            ? "adecuado con oportunidades de mejora"
            : "insuficiente para contextos operacionales de alta exigencia"} en las competencias evaluadas.
...
El perfil evaluado evidencia ${input.score >= 85

...
async function renderReportHTML(data) {
    const templatePath = path_1.default.join(__dirname, "..", "templates", "reportTemplate.html");
    let html = fs_1.default.readFileSync(templatePath, "utf-8");
    const participant = data.participant || {};
    console.log("PARTICIPANT PDF:", participant);
    const participantProfile = participant.perfil ||
        participant.profile ||
        "";
    const competencies = data.competencies || [];
    const evaluationName = evaluationLabels[data.type] ||
        data.evaluationName ||
        "";
    /* ======================================
    FECHA REAL
  ...
  `;
    ...
    ANALISIS
    ====================================== */
    const analysis = clean(data.analysis ||
        data.aiText ||
        "");
    /* ======================================
    REEMPLAZOS
    ...
    html = html
        ...
        .replace(/{{profile}}/g, participantProfile)
        .replace(/{{Profile}}/g, participantProfile)

...
import { generateAIReport, evaluateCompetencyAI } from "./aiEngine"
...
SECURITY SCORE
====================================== */
function calculateSecurityScore(answers:any[]){
...
export async function generateEvaluationReport(sessionId: string){
  ...
  if(session.evaluation.type === "ICOM"){
    ...
    for(const a of answers){
      ...
      const r = await evaluateCompetencyAI(
        a.question.text,
        a.answer || "",
        Array.isArray(a.question.keywordsJson)

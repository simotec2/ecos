...
export async function generateEvaluationReport(sessionId: string) {
  ...
  if (type === "PETS") {
    ...
    const results = await Promise.all(
      ...
      session.answers.map(async (a) => {
        ...
        return {
          competency:
            safeText(a.question.competency) || "General",
    ...
    competencies = Object.entries(grouped).map(
      ([name, arr]) => ({
  ...
  const traffic =
    calculateTrafficLight(score)

  const enriched =
    ai.enrichCompetencies(clean)
  ...
  const answersText = session.answers
    ...
    .map((a) => ({
      ...
      competency:
        safeText(a.question.competency)
  ...
  const recommendations =
    ai.generateRecommendations(enriched)

  const risk =
    ai.calculateRisk(score)
